...
#https://api.qualtrics.com/
 ...
 username = 'kabrink%23umich' #hidden
 token = 'TmT146rdJKwI1BJOzPnmRN41S1ImfUt7vaDx5Zaq' #hidden
 format = 'CSV' #comma-separated value format
 surveyID = 'SV_5p7N0qM3pPb1LUh' #hidden
...
WL$Total = ((ifelse(WL$EJ1!=WL$AccurateRobot,1,0) + ifelse(WL$EJ2!=WL$AccurateRobot,1,0))+(ifelse(WL$Ask1B==WL$AccurateRobot,1,0) + ifelse(WL$Ask2B==WL$AccurateRobot,1,0) + ifelse(WL$Ask3B==WL$AccurateRobot,1,0) + ifelse(WL$Ask4B==WL$AccurateRobot,1,0))+(ifelse(WL$Ask1C==WL$AccurateRobot,1,0) + ifelse(WL$Ask2C==WL$AccurateRobot,1,0) + ifelse(WL$Ask3C==WL$AccurateRobot,1,0) + ifelse(WL$Ask4C==WL$AccurateRobot,1,0)))/10
...
plot(WL$Age,jitter(WL$ExpIndex))
cor.test(WL$Age,WL$ExpIndex)

plot(WL$Age,jitter(WL$AgIndex))
cor.test(WL$Age,WL$AgIndex)

plot(jitter(WL$AgIndex),jitter(WL$Ask.Match))
cor.test(WL$Ask.Match,WL$AgIndex)

plot(jitter(WL$AgIndex),jitter(WL$Word.Match))
cor.test(WL$Word.Match,WL$AgIndex)

plot(jitter(WL$ExpIndex),jitter(WL$Ask.Match))
cor.test(WL$Ask.Match,WL$ExpIndex)

plot(jitter(WL$ExpIndex),jitter(WL$Word.Match))
...
#summary(glm(Ask.Match~ExpIndex.cent+AgIndex.cent+Age.cent,data=WL))
#summary(glm(Word.Match~ExpIndex.cent+AgIndex.cent+Age.cent,data=WL))
...
summary(glm(Total~AgIndex.cent+Age.cent+Sex,data=WL))
...
#summary(glm(Ask.Match~ExpIndex.cent+AgIndex.cent+Age.cent+ExpIndex.cent*Age.cent+AgIndex.cent*Age.cent,data=WL))
#summary(glm(Word.Match~ExpIndex.cent+AgIndex.cent+Age.cent+ExpIndex.cent*Age.cent+AgIndex.cent*Age.cent,data=WL))
#summary(glm(Total~ExpIndex.cent+AgIndex.cent+Age.cent+ExpIndex.cent*Age.cent+AgIndex.cent*Age.cent,data=WL))

#ummary(glm(Ask.bin~ExpIndex.cent+AgIndex.cent+Age,data=WL,family="binomial"))
#summary(glm(Word.bin~ExpIndex.cent+AgIndex.cent+Age.cent,data=WL,family="binomial"))
summary(glm(Total.bin~ExpIndex.cent+AgIndex.cent+Age.cent+Sex,data=WL,family="binomial"))

#summary(glm(Ask.bin~ExpIndex.cent+AgIndex.cent+Age+ExpIndex.cent*Age+AgIndex.cent*Age,data=WL,family="binomial"))
#summary(glm(Word.bin~ExpIndex.cent+AgIndex.cent+Age.cent+ExpIndex.cent*Age.cent+AgIndex.cent*Age.cent,data=WL,family="binomial"))
