...
#https://api.qualtrics.com/
 ...
 token = 'EV1NOq9slwpxgpJuZuOoUl8c9hjmqUGGNPDEz95A' #hidden
...
```{r formatting}
#Clean the data set for analysis
...
WL$Ask.bin = ifelse(WL$Ask.Match <=.5, 0, 1)
...
WL$Word.bin = ifelse(WL$Word.Match <=.5, 0, 1)
...
summary(lm(Ask.Match~ExpIndex+AgIndex+age+Sex,data=WL))
summary(lm(Word.Match~ExpIndex+AgIndex+age+Sex,data=WL))

summary(lm(Ask.bin~ExpIndex+AgIndex+age+Sex,data=WL))
summary(lm(Word.bin~ExpIndex+AgIndex+age+Sex,data=WL))
