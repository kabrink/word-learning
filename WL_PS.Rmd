...
WL1$Total = rowMeans(WL1[c('EJ1.Match','Ask1B.Match','Ask2B.Match','Ask3B.Match','Ask4B.Match','Ask1C.Orange','Ask2C.Orange','Ask3C.Orange','Ask4C.Orange')],na.rm=T)
...
names(WL1.perf) = c("Accuracy","Ask","Endorse","Participant")
...
```{r WL_figure_4_prev_study}

performance.sub = data.frame(`Question Type`=c("Accuracy","Ask","Endorse"),
                             Proportion=c(.71,.70,.67),
                             CI=c(.16,.12,.12))
names(performance.sub) = c("Question Type","Proportion","CI")


performance.sub$Condition = "2 Human Informants"
performance$Condition ="1 Machine Informants"

performance = rbind(performance,performance.sub)


ggplot(data = performance, aes(x = `Question Type`, y = Proportion, group = Condition, fill = `Question Type`)) + 
  guides(fill=FALSE) + 
  scale_fill_manual(values=wes_palette(n=3, name=wesanderson)) +
...
Children’s scores ranged from `r min(WL1$Total)` to `r max(WL1$Total)` (M = `r round(mean(WL1$Total),2)`, SD = `r round(sd(WL1$Total),2)`)
...
WL2$Total = rowMeans(WL2[c('EJ1.Match','Ask1B.Match','Ask2B.Match','Ask3B.Match','Ask4B.Match','Ask1C.Orange','Ask2C.Orange','Ask3C.Orange','Ask4C.Orange')],na.rm=T)
...
#WL2 = WL2[!grepl("Speaks Portuguese at home",WL2$Comments),]
```
...
WL2 = WL2[!grepl("Asked to stop",WL2$Comments),]
...
##Results
###Comparisons with chance for three test questions.
```{r WL_Figure_4_right, fig.height=4.5,fig.width=4.5}
#Compare performance to chance guessing
...
names(WL2.perf) = c("Accuracy","Ask","Endorse","Participant")
...
Children’s scores ranged from `r min(WL2$Total)` to `r max(WL2$Total)` (M = `r round(mean(WL2$Total),2)`, SD = `r round(sd(WL2$Total),2)`)
...
names(WL2sub.perf) = c("Accuracy","Ask","Endorse","Participant")
...
Children were still at chance for Ask, `r mean(WL2.EJ1$Ask.Match,na.rm=T)`, t(`r Ask.ttest$parameter`) = `r round(Ask.ttest$statistic,2)`, p = `r round(Ask.ttest$p.value,4)`, CI 95% [`r paste(round(Ask.ttest$conf.int[1],2), round(Ask.ttest$conf.int[2],2),sep=",")`]; and Endorsement questions, `r mean(WL2.EJ1$Word.Match,na.rm=T)`, t(`r Word.ttest$parameter`) = `r round(Word.ttest$statistic,2)`, p = `r round(Word.ttest$p.value,4)`, CI 95% [`r paste(round(Word.ttest$conf.int[1],2), round(Word.ttest$conf.int[2],2),sep=",")`].
